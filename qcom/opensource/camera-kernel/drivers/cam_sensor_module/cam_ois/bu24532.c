...
static bool bu24532_fw_version_matches(uint32_t curr_fw_version,
	uint32_t expected_fw_version)
{
	uint32_t expected_ver24 = expected_fw_version & 0x00FFFFFF;

	if (curr_fw_version == expected_fw_version)
		return true;

	if ((curr_fw_version & 0x00FFFFFF) == expected_ver24)
		return true;

	if (((curr_fw_version >> 8) & 0x00FFFFFF) == expected_ver24)
		return true;
...
static int cam_bu24532_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (0 == bu24532_ois_fw_update_op){
		...
		if (true == o_ctrl->is_second_init &&
			!bu24532_fw_version_matches(curr_fw_version,
			o_ctrl->opcode.fw_version))
		{
			for (i=0; i< AUTO_BOOT_RETRY_TIME; i++)
			{
				...
				CAM_INFO(CAM_OIS, "[BU24532]read program ID:0x%x, current ID:0x%x",
						curr_fw_version,
						o_ctrl->opcode.fw_version);
				if (bu24532_fw_version_matches(curr_fw_version,
					o_ctrl->opcode.fw_version))
				{
					break;
				}
